...
io.on('connection', (socket) => {
    ...
    socket.on('send-file', function (name, buffer) {
        ...
        var fileName = path.join(process.cwd(), "public", "img", "uploads", name);
...
app.post("/asocations/create/newAs", (req, res) => {
    const A = req.body.A.split(",");
    const B = req.body.B.split(",");
    const C = req.body.C.split(",");
    const D = req.body.D.split(",");
    const P = req.body.P;
    ...
    MongoClient.connect(url, function (err, db) {
        ...
        dbo.collection('asocationsNewGame').find({}).toArray(function (err, result) {
            ...
            var game = data;
            dbo.collection("asocationsNewGame").insertOne(game, function (err, res) {
                if (err) throw err;
                db.close();
            });
            db.close();
...
app.get("/asocations/create", (req, res) => {
    res.render('../public/asocations/create')
})

app.get("/asocations/game/:id", (req, res) => {
